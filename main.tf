  ...
  metadata = {
    windows-startup-script-ps1 = <<EOF
      ...
      if (!(Test-Path $flagFile)) {
          ...
          # 1. Create or Reset 'adminuser'
          if (Get-LocalUser -Name "adminuser" -ErrorAction SilentlyContinue) {
              net user adminuser "${var.admin_password}" /active:yes
          } else {
              $encPass = ConvertTo-SecureString "${var.admin_password}" -AsPlainText -Force
              New-LocalUser -Name "adminuser" -Password $encPass -FullName "Slingshot Admin"
          }
