...
for ZONE in "${ZONE_SEARCH[@]}"; do
  if gcloud compute instances describe "$VM_NAME" \
      --zone="$ZONE" \
      --project="$USER_PROJECT" &>/dev/null; then
    VM_ZONE="$ZONE"
    break
  fi
done
...
if [ -z "$VM_ZONE" ]; then
  echo "ERROR: Could not find VM in any us-central1 zone."
  exit 1
fi
...
# ─── STEP 10: REMOVE IAM BINDING ─────────────────────────────────────────────

echo ""
echo ">>> Removing Slingshot Manager IAM binding..."
if ! gcloud projects remove-iam-policy-binding "$USER_PROJECT" \
  --member="serviceAccount:$MANAGED_SA" \
  --role="roles/compute.instanceAdmin.v1" \
  --quiet > /dev/null 2>&1; then
  echo "WARNING: IAM binding removal failed or was already absent."
else
  echo "IAM binding removed."
fi
