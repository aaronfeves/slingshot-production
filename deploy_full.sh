...
# ─── 6. INFRASTRUCTURE DEPLOYMENT (Robust Mode) ──────────────────────────────

echo ">>> Applying Infrastructure in zone $DEPLOY_ZONE..."
...
VM_ZONE="$DEPLOY_ZONE"
echo ""
echo "VM deployed in zone: $VM_ZONE"
...
if true; then
  ...
  # ─── 8. WRITE VM CONFIG TO GCS ──────────────────────────────────────────────

  echo ""
  echo ">>> Saving VM configuration..."

  DISK_NAME=$(gcloud compute instances describe "$SERVER_NAME" \
    --zone="$VM_ZONE" \
    --project="$GOOGLE_CLOUD_PROJECT" \
    --format="value(disks[0].source)" | awk -F'/' '{print $NF}')

  EXTERNAL_IP=$(gcloud compute instances describe "$SERVER_NAME" \
    --zone="$VM_ZONE" \
    --project="$GOOGLE_CLOUD_PROJECT" \
...
EOF
  ...
  # ─── 9. REPLACE TERRAFORM SCHEDULE WITH STOP-ONLY ──────────────────────────
  ...
  # ─── 10. CREATE CLOUD SCHEDULER START JOB ───────────────────────────────────
